---
title: "Cognitive Science"
summary: "What is it? + Coursework"
date: "July 11, 2025"
draft: false
tags:
- Machine Learning
- Artificial Intelligence
---

## What is Cognitive Science?
Whenever I tell someone that I am minoring in Cognitive Science, they always ask me, **what even _is_ cognitive science?** This is a completely valid question, and before coming to UCSD I probably would've called it a fancy term for _psychology_. From my experience, there is no singular way to define what cognitive science is. I know-this sounds cliche-but cognitive science is such an interdisciplinary subject that your perspective of cognitive science really depends on your specialization. However, for the sake of answering the question-based on many definitions provdided over several classes, 

**I define cognitive science as the intersection of 5 key domains:**
- Neuroscience
- Psychology
- Linguistics
- Philosophy
- Computer Science

**with the goal of being able to understand AND model cognition, perception, language and consciousness through exploration of brain function, AI, language models, and interactions with the world.** This definition is a lot to take in, but this is essentially the core of cognitive science; much more than _just_ fancy psychology, now is it? 
## Why Major/Minor in Cognitive Science? 
1. Minoring in cognitive science as a data science student helps me understand how I can apply my knowledge of software and coding to other fields of critical cutting-edge research, especially in AI & LLMs [intersection of linguistics & computer science] and Data Ethics [intersection of philosophy & computer science]. 
2. From a logistical standpoint, UCSD's Cognitive Science and Data Science (as well as Computer Science) curriculum have significant overlap, ideally allowing me to take the minor without delaying my graduation.
3. UCSD hosts the world's first and one of the best cognitive science departments. Why not learn something your school is a pioneer in & excels at?
## My reccomendation: 
If you think your degree plan can handle the extra workload, I'd reccomend checking some of the classes in the department out. You don't even have to minor or major in the department, you could just take some gen-eds to get introduced to the subject.


## My Cognitive Science Coursework
- ### COGS 1: Introduction to Cognitive Science
  - **Professor:** Mary Boyle
  - **Grade:** A
  - **Workload:** Not Much
  - **When** - Spring 2024
  - **My Thoughts:**
    - I loved how this class introduced so many areas of cognitive science, from sleep to AI, through talks by different professors and researchers.
    - I actually enjoyed that participation could waive the final—though our section almost didn’t make the cut (maybe partly my fault).
    - This class helped me figure out which professors I wanted to take later; it’s why I ended up in COGS 100 and COGS 150.

- ### COGS 8: Hands-On Computing
  - **Professor:** Kyle Shannon
  - **Grade:** A
  - **Workload:** Not Much
  - **When** - Fall 2023
  - **My Thoughts:**
    - I found this class to be pretty light but super helpful, especially as a freshman trying to get settled at UCSD.  
    - The small class size was a nice change from typical tech courses, with most time spent on small-group activities, which made it really easy to meet people, particular Professor Kyle and TA Akshay Nagarajan, both of whom I ended up working with in the future.  
    - It was my first time working with a MicroBit and actually using Python for hardware projects instead of just making calculators or simple software.  

    - **Projects**:
      - **Inchworm:** Built little inch-worm robots out of cardboard, paperclips, motors, and the MicroBit, then raced them across the classroom.  
      - **Speedway Robot:** Used live camera feeds to steer the robot based on trained signatures for obstacles, track lines, and objectives.

- ### COGS 9: Introduction to Data Science
  - **Professor:** Kyle Shannon
  - **Grade:** A
  - **Workload:** Not Much
  - **When** - Spring 2024
  - **My Thoughts:**
    - I saw this as a very light course, a gentle introduction to data science that focused more on the *purpose* of data science as a tool than on heavy technical details.  
    - The class doesn’t dive deep into coding but instead lays down foundational ideas like data ethics and the scientific process, which I think most people completely overlook.  
    - Kyle made it genuinely interesting with live demos and special lectures that went beyond the standard material. One standout was when he presented the results of a data science project, which helped me appreciate how crucial communication is in a corporate environment, especially when you’re the only data scientist in the room.  
    - **TUTOR PERSPECTIVE:** A year later, I got to come back as a tutor for this class, running demos on SQL, Python, and even a bit of Reinforcement Learning. I loved sitting in on lectures again (mainly just to hang out with Kyle) and revisiting material from sessions I totally didn’t skip the first time. It’s a bit of a shame more students don’t show up, since there’s so much potential for this class to be fun, but low engagement really holds it back.

- ### COGS 14A: Introduction to Research Methods
  - **Professor:** James Barrera
  - **Grade:** A-
  - **Workload:** Decent
  - **When** - Spring 2024
  - **My Thoughts:**
    - I took this class mainly because it’s part of the "Machine Learning & Artificial Intelligence" domain track for the Data Science major. While it does touch on foundations relevant to ML and AI from a stats perspective, most of the material is actually geared toward neuroscience research, which isn’t a bad thing, just something I noticed.  
    - If you’ve taken AP Stats, the first half is pretty smooth sailing. The second half is more about memorizing different study designs, like group-subject tests and brain imaging methods.  
    - I wouldn’t call this class essential for understanding machine learning, but it absolutely makes sense in the curriculum. If you plan to do any research, especially in bio, neuro, cog sci, or stats-heavy labs, this stuff is critical.  
    - I also have to say Professor Barrera was fantastic. I had some great post-lecture chats with him; he was always up for questions, clearly passionate about the material, and gave genuinely helpful career advice for anyone thinking about R&D.

- ### COGS 14B: Introduction to Statistical Analysis
  - **Professor:** Drew Ellen Hoffman
  - **Grade:** A+
  - **Workload:** Decent
  - **When** - Spring 2024
  - **My Thoughts:**
    - This class was honestly pretty easy for me since it mostly felt like a repeat of AP Stats, just with a few new topics sprinkled in.  
    - Professor Hoffman was amazing, super approachable and always happy to answer questions after class, much like Professor Barrera in 14A. That said, she could be a bit disorganized with homework deadlines, so it helped to stay on top of things yourself.  
    - It was cool to learn a new statistical software package for homework, though at the end of the day the TI-84 was still my best friend for getting through the class.


- ### COGS 100: Cyborgs Now & In the Future
  - **Professor:** Michael Allen Gordon
  - **Grade:** A
  - **Workload:** Decent
  - **When** - Summer 2024
  - **My Thoughts:**
    - This class was something for sure.  
    - COGS 100 dives deep into cognitive science, almost like a mashup of history, psychology, neuroscience, machine learning, and philosophy, which makes sense since it’s a core course for the Cog Sci major.  
    - Some parts were kinda boring. It’s super lecture-heavy and a few of those dragged on forever (though to be fair, I took it over summer and was mostly watching recordings).  
    - That said, a lot of the topics are genuinely fascinating, like cognitive offloading, niche construction, and the origins of the artificial neuron that inspired today’s neural networks.  
    - If you’re a Cog Sci major or minor, you’ll have to take this anyway. But even if you’re not, it’s a solid pick for an upper-div GE, pretty interesting and not heavy on homework.

- ### COGS 101C: Language
  - **Professor:** Alyssa Truman
  - **Grade:** A
  - **Workload:** Decent
  - **When** - Summer 2024
  - **My Thoughts:**
    - I found this class super interesting, it dove into linguistics and the history of language from both biological and cultural perspectives, which honestly wasn’t something I ever expected to study.  
    - I took it over the summer, and while it wasn’t particularly difficult, there *was* a lot of new vocabulary to pick up.  
    - The material was genuinely fascinating though, and I’d actually recommend this over COGS 101A or B, even if it’s supposed to be the harder one of the three.

- ### COGS 108: Data Science in Practice
  - **Professor:** Jason Fleischer
  - **Grade:** A
  - **Workload:** Decent
  - **When** - Fall 2024
  - **My Thoughts:**
    - This class basically runs parallel to DSC 80, but without the heavy theory, it’s all about getting exposed to a wide range of implementation libraries.  
    - It’s honestly beyond easy, especially since it’s project-based. As long as you have a good group, it’s a breeze.  
    - If you’ve never done EDA or ML before, there might be a slight learning curve, but the lack of theory keeps it manageable.
    - Professoir Fleischer is great but his TA staff was also excellent, particularly Scott Yang.

- ### COGS 118A: Introduction to Supervised Machine Learning
  - **Professor:** Anjum Gupta
  - **Grade:** IP
  - **Workload:** Decent
  - **When** - Summer 2025
  - **My Thoughts:**

- ### COGS 118B: Introduction to Unsupervised Machine Learning
  - **Professor:** Anjum Gupta
  - **Grade:** A
  - **Workload:** Decent
  - **When** - Summer 2024
  - **My Thoughts:**
    - This class had a *steep* learning curve for me since I somehow took it with only 2 out of the 7 recommended prereqs. The class itself wasn’t inherently hard, but not being up to speed on the math—like not even knowing what a Poisson distribution was—definitely made it tougher.  
    - The tests were pretty challenging and ended up curved down in the summer, apparently due to potential cheating, even though the exams were synchronous (just online).  
    - I wasn’t a big fan of the teaching style. Professor Gupta mostly played chopped up pre-recorded videos from past lectures, which wasn’t super engaging (though some of the animations were admittedly cool). He was really chill and approachable to talk to outside of lectures—just not my favorite format for learning.

- ### COGS 150: Large Language Models in Cognitive Science
  - **Professor:** Ben Bergen
  - **Grade:** P
  - **Workload:** Decent
  - **When** - Summer 2024
  - **My Thoughts:**
    - This class was excellent, but honestly there should be more formal prereqs, having taken something like DSC 40A or COGS 118A/185 beforehand will really help build intuition for the math you dive into.  
    - I took it P/NP since the course was in-person while I was remote, so I basically had to accept a zero on the 25% participation grade.  
    - The final project was actually an exam where we had to defend our position on whether or not AI can “think,” which was super thought-provoking.  
    - The course material itself was very interesting, starting from the historical foundations of LLMs, moving through transformers and their architectures, and exploring their impact on cognitive science.

- ### COGS 185: Advanced Machine Learning Methods
  - **Professor:** Zhouwen Tu
  - **Grade:** A
  - **Workload:** High
  - **When** - Spring 2025
  - **My Thoughts:**
    - Zhouwen Tu was an excellent professor who somehow managed to pack an insane amount of material into this course.  
    - My final project (with my partner Jeronimo) involved comparing image generation models, we even pulled an all-nighter to get it done, but it was totally worth it because the project turned out great.  
    - This is truly an *advanced* course, and for a COGS class it’s definitely tough conceptually. It covered everything from SVMs to cutting-edge generative models, and was impressively up to date with the latest AI tech.

- ### COGS 186: Genetics Algorithms
  - **Professor:** Anjum Gupta
  - **Grade:** 
  - **Workload:** 
  - **When** - Summer 2025
  - **My Thoughts:**

- ### COGS 188: AI Algorithms
  - **Professor:** Jason Fleischer
  - **Grade:** A
  - **Workload:** High
  - **When** - Winter 2025
  - **My Thoughts:**
    - This class started by covering a bunch of crucial concepts and algorithms, like A*, that honestly should be required in DSC courses but somehow aren’t.  
    - The textbook and readings were incredibly detailed and well-chosen, and Professor Fleischer did his best boiling everything down in the limited lecture time, though it just wasn't enough time. Not the professor's fault, the material was just too dense. However, he was very open to discussing concepts after class and in OH.  
    - It was project-based (thankfully), which helped me land an A, but the amount of theory was so intense that if it were exam-heavy I probably would’ve been cooked. I still don’t fully grasp some of the math behind reinforcement learning, but I’ve got the readings downloaded and plan to work through them eventually.  
    - Also kind of funny: for some reason there was a really heavy emphasis on reinforcement learning, even though there’s a separate class for that.
